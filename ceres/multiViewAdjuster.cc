...
int main(int argc, char** argv){
	...
	for(int i=0;i<numViews;i++){
		...
		for(int j=0;j<numPts;j++){
			...
			// if(i==37&&j==35) {
			// 	std::cout<<curEigVec[0]<<" "<<curEigVec[1]<<" "<<curEigVec[2]<<std::endl;
			// }
		...
		// // Create a cost function to restrict translation of vehicles at a frame with respect to the same in previous frame.
		// ceres::CostFunction *multiViewTranslationRegularizer = new ceres::AutoDiffCostFunction<MultiViewTranslationRegularizer, 3, 3>(
		// 	new MultiViewTranslationRegularizer(trans + 3*(i-1)));
		// // Add a residual block to the problem
		// shapeProblem.AddResidualBlock(multiViewTranslationRegularizer, new ceres::HuberLoss(0.1), trans + 3*i);
	...
	*/
		...
		// shapeProblem.SetParameterBlockConstant(lambdas);
...
*/
	...
	/*for(int i = 1; i < numViews; ++i){
		// Rotations between successive views must be very less
		ceres::CostFunction *rotationChange = new ceres::AutoDiffCostFunction<RotationChange, 3, 3, 3>(
			new RotationChange(500));
		shapeProblem.AddResidualBlock(rotationChange, NULL, rotAxis + 3*(i-1), rotAxis + 3*i);
		// Translations between successive views must be very less
		ceres::CostFunction *translationChange = new ceres::AutoDiffCostFunction<TranslationChange, 3, 3, 3>(
			new TranslationChange(500/(i+1)));
		shapeProblem.AddResidualBlock(translationChange, NULL, trans + 3*(i-1), trans + 3*i);
	}*/
