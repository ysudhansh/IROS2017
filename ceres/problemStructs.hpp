...
public:
	...
	int getNumVecs() const { return numVecs_;}
	// Return the center of the car
	double* getCarCenter() { return carCenter_; }
	// Return the height of the car
	double getCarHeight() const { return h_; }
	...
	bool loadFile(const char *fileName){
		...
		fscanfOrDie(fptr, "%d", &numPts_);
		fscanfOrDie(fptr, "%d", &numObs_);
		fscanfOrDie(fptr, "%d", &numVecs_);

		// std::cout<<numViews_<<" "<<numPts_<<" "<<numObs_<<std::endl;
		...
		// for(int i=numViews_-1;i<numViews_;i++){
		// 	for(int j = 41; j < 42; ++j){
		// 		for(int k = numPts_-1; k < numPts_; ++k){
		// 			std::cout << V_[i*3*42*numPts_ + j*3*numPts_ + 3*k + 0] << " " << V_[i*3*42*numPts_ + j*3*numPts_ + 3*k + 1] << " " << V_[i*3*42*numPts_ + j*3*numPts_ + 3*k + 2] <<std::endl;
		// 		}
		// 	}
		// }
		...
		// std::cout << trans_[3*numViews_-3] << " "  << trans_[3*numViews_-2] <<" "  << trans_[3*numViews_-1]<<std::endl;
